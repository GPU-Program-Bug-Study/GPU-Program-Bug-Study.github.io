    ...
    if type(i) is FunctionSignature:
      ...
      #print(gpu_result)
        ...
        #double_gpu_err, double_g_result = verify_gpu_result(double_gpu_result)
          ...
          if gpu_err==1 or cpu_err == 1 or (cpu_err == 1 and gpu_err == 1):
        ...
        #print("Checking for Rounding Error", file=log_file)
          ...
          round_err, r_flag = check_for_rounding_errors(g_result, c_result)

          if round_err == 1:
            print("A rounding error has been detected. No further analysis",file=log_file)
            continue

          envr_err, env_flag = check_for_environmental_errors(gpu_elapsed, cpu_elapsed)

          if envr_err == 1:
            print("An environmental error has been detected. No further analysis",file=log_file)
            continue

          data_type_err, data_flag = check_for_datatype_errors(gpu_result, double_gpu_result, gpu_elapsed, double_gpu_elapsed)
          if data_type_err == 1:
            print("A data type error has been detected. No further analysis",file=log_file)
            continue
